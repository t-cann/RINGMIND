...
final float scale = 50/Rp;        // Converts from [m] to [pixel] with planetary radius (in pixels) equal to the numerator. Size of a pixel represents approximately 600km.
...
class RingSystem {
  ...
  RingSystem() {
    ...
    moons.add(new Moon(G*3.7e19, 2.08e6, 3.174802*Rp, color(255,0,0)));
    moons.add(new Moon(G*3.7e19, 2.08e6, 5.0396842*Rp, color(0,255,0)));
    moons.add(new Moon(G*3.7e19, 2.08e6, 8*Rp, color(0,0,255)));
    //addMoon(5, moons);
    //addMoon(6, moons);
    //addMoon(9, moons);
    //addMoon(12, moons);
