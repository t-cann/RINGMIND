...
class Grid {
  ...
  /**
   * Returns radius of center of the cell (from x=0 , y=0) 
   **/
  ...
  /**
   *  Return angle from ... to centre of the cell.
   **/
  float angleCell(int i) {
    return radians(dtheta*(i+0.5) -180);
  }

  float diffAngle(int i, PVector position) {
    return angleCell(i)-atan2(position.x, position.y);
  }
