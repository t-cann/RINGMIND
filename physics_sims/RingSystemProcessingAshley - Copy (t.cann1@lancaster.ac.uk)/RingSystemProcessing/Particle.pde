...
abstract class Particle {
   ...
   */
  Particle(float r, float phi ,float theta) {
    //this(r*cos(phi), r*sin(phi), 1E7*randomGaussian(), sqrt(GMp/(r))*sin(phi), -sqrt(GMp/(r))*cos(phi), 0);
    
    this(r*sin(theta)*cos(phi), r*sin(theta)*sin(phi), r*cos(theta) , sqrt(GMp/(r))*sin(phi), -sqrt(GMp/(r))*cos(phi), sqrt(GMp/(r))*0);
  }
