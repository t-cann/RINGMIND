...
class ShearingBox {
  ...
  void update() {
    ...
    for (Particle p : particles) {
      ...
      int x = (int(p.x) +Lx/2)/ scl;   //
      int y = (int(p.y) +Ly/2)/scl;     //
  ...
  void collision(Particle p, Particle other) {
    ...
    // Get distances between the balls components
                PVector distanceVect = PVector.sub(new PVector(p.x,p.y),new PVector(other.x,other.y));
                ...
                if (distanceVectMag < minDistance) {
                  ...
                  p.x -= correctionVector.x;
                  p.y -= correctionVector.y;
                  ...
                  vTemp[0].x  = cosine * p.vx + sine * p.vy;
                  vTemp[0].y  = cosine * p.vy - sine * p.vx;
                  vTemp[1].x  = cosine * other.vx + sine * other.vy;
                  vTemp[1].y  = cosine * other.vy - sine * other.vx;
                  ...
                  other.x = p.x + bFinal[1].x;
                  other.y = p.y + bFinal[1].y;

                  p.x += bFinal[0].x;
                  p.y += bFinal[0].y;

                  // update velocities
                  p.vx = cosine * vFinal[0].x - sine * vFinal[0].y;
                  p.vy = cosine * vFinal[0].y + sine * vFinal[0].x;
                  other.vx = cosine * vFinal[1].x - sine * vFinal[1].y;
                  other.vy = cosine * vFinal[1].y + sine * vFinal[1].x;
                }
  }
   ...
   *   TODO: Check Algorithm is correct.
   */
  void step_verlet(ArrayList<Particle> p) {
