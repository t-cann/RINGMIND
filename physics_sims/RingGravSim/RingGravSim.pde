...
ax_grav = lambda _x,_y,_GM: -_GM*_x/pow(sqrt(sq(_x) + sq(_y)), 3.0);
ay_grav = lambda _x,_y,_GM: -_GM*_y/pow(sqrt(sq(_x) + sq(_y)), 3.0);
...
v_orboid[0,:] = sqrt(vx_orboid[0:10]**2+vy_orboid[0:10]**2);
...
for i in range(n_steps):
  ...
  x_orboid = x1_orboid
  y_orboid = y1_orboid
  v_orboid[i+1,:] = sqrt(vx_orboid[0:10]**2+vy_orboid[0:10]**2)
