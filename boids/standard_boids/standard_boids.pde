...
//  boids.add(new Boid(100 + random(100), 100 + random(100)));
...
void setupDoubleCircle() {
  ...
  for (int x = 0; x < 25; x+= 1) {
     float dir = (x / 25.0) * TWO_PI;
    avoids.add(new Avoid(width * 0.5 + cos(dir) * height*.2, height * 0.5 + sin(dir)*height*.2));
  } 
}
